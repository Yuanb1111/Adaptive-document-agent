...
### v31: staged analysis and repeat exports

- Candidate selection and semantic scoring now share one model request. Deterministic evidence checks still gate execution. Every generated candidate retains a score/rejection record, including candidates outside the review budget and missing model decisions.
- Discovery merge input uses an exact-term catalog with source-chunk/page mappings, retaining summaries, every metric, and conflicting quality notes. Python does not merge synonymous terms. The merged profile retains the discovered vocabulary even if the model omits an item.
- **Stage models (optional)** in the sidebar reads the `LLM_*_MODEL` overrides below. Blank fields use the main model; changing providers does not carry the previous provider's defaults. No faster model is selected automatically. Local stage overrides remain routed to the local provider/endpoint.
- Report-outline planning and PPT question selection consume the same profile, evidence, and insights independently. They overlap only for opted-in cloud clients; local/stateful clients stay serial. Chart and final slide planning still wait for their evidence dependencies.
