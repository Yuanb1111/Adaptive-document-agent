...
- Rendering uses an administrator-configured **local** Node.js + `@oai/artifact-tool` installation. Set `PPTX_QA_NODE` and `PPTX_QA_ARTIFACT_MODULE` to absolute local paths (see `.env.example`), then restart the app. These are optional external runtime dependencies, not installed by `pip`. Without them, verified PowerPoint export is blocked explicitly; other export formats remain available. Never set executable/module paths from document content or an untrusted upload.
