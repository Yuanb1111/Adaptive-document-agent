    ...
    def analyse_pdf(
        ...
        *,
        ...
        with record_timing(timings, "table_extraction"):
            ...
            cached_tables = self.cache.get_model(f"tables-v7-{digest}-{scope}", ParsedDocument) if self.cache else None
            if cached_tables is not None:
                ...
                if self.cache:
                    self.cache.set_model(f"tables-v7-{digest}-{scope}", document)
