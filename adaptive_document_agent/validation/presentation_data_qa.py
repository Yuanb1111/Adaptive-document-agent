    ...
    for slide in result.presentation_plan.slides:
        ...
        for cid in chart_ids:
            ...
            if chart:
                ...
                chart_topic = slide.model_copy(update={"title": chart.title, "message": "", "section_title": "", "bullets": []})
                if any(is_positive_topic_mismatch(observations[oid], chart_topic)
                    ...
                    issues.append(QAItem(code="chart_title_data_mismatch", severity="CRITICAL",
                        slide_id=slide.id, related_ids=[cid], message=f"Chart '{chart.title}' does not match its linked metric data."))
        ...
        if any(is_positive_topic_mismatch(observations[oid], slide) for oid in block_ids if oid in observations):
